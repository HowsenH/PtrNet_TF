...
class TSPDataLoader(object):
  def __init__(self, config, rng=None):
    ...
    print("lalalla path:{}".format(paths))
    if len(paths) != 0:
...
class MyTSPDataLoader(object):
  def __init__(self, config, rng=None):
    ...
    self.read_update_data(config.x_path, config.y_path, self.data_num['test'])
    self._create_input_queue()

  def read_update_data(self, x_path, y_path, test_size):
    x = np.load(x_path)
    # Plus one fore index miss matching
    y = np.load(y_path) + 1

    if self.data is None:
      self.data = {}

    tf.logging.info("Update data with x-path:{} and y-path:{} stored locally with test size {}".format(x_path, y_path, test_size))
    self.data['train'] = TSP(x=x[:test_size], y=y[:test_size], name='train')
    self.data['test'] = TSP(x=x[test_size:], y=y[test_size:], name='test')
  ...
  def _create_input_queue(self, queue_capacity_factor=16):
    ...
    for name in self.data_num.keys():
      ...
      self.queue_ops[name] = tf.RandomShuffleQueue(
          ...
          min_after_dequeue=min_after_dequeue,
          dtypes=[tf.float32, tf.int32],
          # Changed input shape here
          shapes=[[self.max_length, self.max_length,], [self.max_length]],
          seed=self.random_seed,
          name="random_queue_{}".format(name))
      self.enqueue_ops[name] = \
          self.queue_ops[name].enqueue([self.input_ops[name], self.target_ops[name]])
